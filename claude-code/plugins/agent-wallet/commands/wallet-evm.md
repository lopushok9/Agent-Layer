...
This resolves the current EVM wallet session and returns the active EVM network. Use the returned `selected_network` in the final report.
...
- include `chain`, the active EVM network, `address`, and `total_value_usd` when present
...
5. Be explicit that this command shows one EVM network at a time: the active network returned by `set_wallet_backend`.

6. Do not suggest transfers, swaps, or other write actions unless the user explicitly asks for them.
