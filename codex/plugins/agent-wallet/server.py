...
def _normalize_wallet_backend(value: Any) -> str:
    ...
    if backend not in BACKENDS:
        raise RuntimeError("Wallet backend must be solana, evm, ethereum, base, robinhood, arc, btc, or bitcoin.")
    return backend
...
def _normalize_selectable_evm_network(value: Any) -> str:
    ...
    if network in {"sepolia", "base-sepolia", "base_sepolia"}:
        raise RuntimeError("EVM testnets are no longer supported. Use ethereum, base, robinhood, or arc.")
    if network not in {"ethereum", "base", "robinhood", "arc"}:
        raise RuntimeError("EVM network must be 'ethereum', 'base', 'robinhood', or 'arc'.")
    return network
...
def _implied_evm_network_from_backend_alias(value: Any) -> str | None:
    ...
    if normalized == "robinhood":
        return "robinhood"
    if normalized in {"arc", "arc-mainnet"}:
        return "arc"
...
def _default_evm_network() -> str | None:
    ...
    if configured in {"ethereum", "base", "robinhood", "arc"}:
        return configured
    ...
    return [
        {
            ...
            "input_schema": {
                ...
                "properties": {
                    "backend": {
                        "type": "string",
                        "description": "solana, evm, ethereum, base, robinhood, arc, btc, or bitcoin.",
                    },
                    "network": {
                        "type": "string",
                        "description": "Optional network override. Use ethereum, base, robinhood, or arc for EVM.",
                    },
                    "address": {
                        "type": "string",
        ...
        },
        {
            ...
            "description": (
                "Set the active EVM network for this Codex MCP session to ethereum, base, robinhood, or arc."
            ),
            "input_schema": {
                "type": "object",
                "properties": {
                    "network": {
                        "type": "string",
                        "description": "ethereum, base, robinhood, or arc.",
                    }
                },
                "required": ["network"],
                "additionalProperties": False,
