...
test("OAuth authenticates first and grants a requesting client only after explicit one-time consent",async()=>{
  ...
  try{
    ...
    assert.equal(authorize.status,200);assert.equal(authorize.headers.get("set-cookie"),null);assert.match(page,/Continue with GitHub/);assert.match(page,/color:#171717!important/);assert.match(page,/Untrusted &lt;script&gt;alert\(1\)&lt;\/script&gt;/);assert.doesNotMatch(page,/<script>alert/);
    ...
    assert.equal(callback.status,200);assert.equal(callback.headers.get("location"),null);assert.match(consentPage,/attacker\.example/);assert.match(consentPage,/Allow access/);assert.match(consentPage,/Cancel/);
...
test("OAuth denial returns access_denied and cannot be replayed",async()=>{
  ...
  const app=express();app.use(oauthRouter(config,store,tokens));app.use((_error:unknown,_req:express.Request,res:express.Response,_next:express.NextFunction)=>res.status(400).json({error:"invalid_request"}));
  const server=app.listen(0,"127.0.0.1");await new Promise<void>(resolve=>server.once("listening",resolve));const base=`http://127.0.0.1:${(server.address() as AddressInfo).port}`;
  try{const response=await fetch(`${base}/oauth/consent`,{method:"POST",body:new URLSearchParams({consent_token:"deny-token",decision:"deny"}),redirect:"manual"});assert.equal(response.status,302);const redirect=new URL(response.headers.get("location")!);assert.equal(redirect.searchParams.get("error"),"access_denied");assert.equal(redirect.searchParams.get("state"),"client-state");const replay=await fetch(`${base}/oauth/consent`,{method:"POST",body:new URLSearchParams({consent_token:"deny-token",decision:"deny"}),redirect:"manual"});assert.equal(replay.status,400);}finally{await new Promise<void>((resolve,reject)=>server.close(error=>error?reject(error):resolve()));}
});
