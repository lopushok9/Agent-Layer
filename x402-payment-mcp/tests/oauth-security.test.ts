...
test("OAuth authenticates first and grants a requesting client only after explicit one-time consent",async()=>{
  ...
  try{
    ...
    assert.equal(authorize.status,200);assert.equal(authorize.headers.get("set-cookie"),null);assert.match(authorize.headers.get("content-security-policy")??"",/script-src 'nonce-/);assert.match(authorize.headers.get("content-security-policy")??"",/img-src https:\/\/www\.agent-layer\.tech/);assert.match(authorize.headers.get("content-security-policy")??"",/form-action 'none'/);assert.match(page,/AgentLayer/);assert.match(page,/<img src="https:\/\/www\.agent-layer\.tech\/android-chrome-512x512\.png" alt="">/);assert.match(page,/Continue with GitHub/);assert.match(page,/Opening "/);assert.match(page,/color:#111!important/);assert.match(page,/Untrusted &lt;script&gt;alert\(1\)&lt;\/script&gt;/);assert.doesNotMatch(page,/<script>alert/);
    ...
    assert.equal(callback.status,200);assert.equal(callback.headers.get("location"),null);assert.match(callback.headers.get("content-security-policy")??"",/form-action 'self' https:\/\/attacker\.example/);assert.match(consentPage,/attacker\.example/);assert.match(consentPage,/Allow access/);assert.match(consentPage,/Connecting to the MCP client/);assert.match(consentPage,/Cancel/);
