...
test("OAuth authenticates first and grants a requesting client only after explicit one-time consent",async()=>{
  ...
  try{
    ...
    assert.equal(authorize.status,200);assert.equal(authorize.headers.get("set-cookie"),null);assert.match(authorize.headers.get("content-security-policy")??"",/script-src 'nonce-/);assert.match(authorize.headers.get("content-security-policy")??"",/form-action 'none'/);assert.match(page,/AgentLayer/);assert.match(page,/<svg aria-hidden="true"/);assert.match(page,/Continue with GitHub/);assert.match(page,/Opening "/);assert.match(page,/color:#111!important/);assert.match(page,/Untrusted &lt;script&gt;alert\(1\)&lt;\/script&gt;/);assert.doesNotMatch(page,/<script>alert/);
