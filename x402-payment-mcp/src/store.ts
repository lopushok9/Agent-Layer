...
export class Store {
  ...
  async registerClient(clientName: string, redirectUris: string[]): Promise<OAuthClient> {
    ...
    const row = await this.pool.query(`INSERT INTO oauth_clients (client_id,client_name,redirect_uris) VALUES ($1,$2,$3) RETURNING client_id,client_name,redirect_uris`, [clientId, clientName, redirectUris]);
  ...
  async createPreview(p:Omit<Preview,"id"|"expiresAt">,ttlSeconds:number){const id=randomUUID();const r=await this.pool.query(`INSERT INTO payment_previews(id,user_id,method,url,request_body,fingerprint,amount,pay_to,expires_at) VALUES($1,$2,$3,$4,$5,$6,$7,$8,now()+($9*interval '1 second')) RETURNING expires_at`,[id,p.userId,p.method,p.url,p.body,p.fingerprint,p.amount,p.payTo,ttlSeconds]);return{id,expiresAt:r.rows[0].expires_at as Date};}
