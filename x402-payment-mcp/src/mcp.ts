...
export function createUserMcp(userId:string,payments:PaymentService){
  ...
  server.registerTool("x402_preview",{title:"Preview x402 payment",description:"Probe a service returned by x402_search and create a short-lived preview bound to the request, selected scheme, and payment terms. This does not sign or spend.",inputSchema:z.object({service_ref:z.string().min(20),method:z.enum(["GET","POST"]).default("GET"),body:z.unknown().optional(),scheme:z.enum(["auto","exact","upto"]).default("auto")}),annotations:{readOnlyHint:true}},async({service_ref,method,body,scheme})=>{try{return text(await payments.preview(userId,service_ref,{method,body},scheme));}catch(e){return failure(e);}});
