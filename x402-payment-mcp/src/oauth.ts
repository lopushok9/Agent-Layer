...
const BRAND_LOGO_URL = "https://www.agent-layer.tech/android-chrome-512x512.png";
...
function secureHtml(res:Response,nonce:string,formAction:string){return res.set("Cache-Control","no-store").set("Content-Security-Policy",`default-src 'none'; img-src https://www.agent-layer.tech; style-src 'unsafe-inline'; script-src 'nonce-${nonce}'; form-action ${formAction}; frame-ancestors 'none'; base-uri 'none'`).set("X-Frame-Options","DENY").set("Referrer-Policy","no-referrer").type("html");}
