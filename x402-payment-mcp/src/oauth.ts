...
function loginPage(input:{stateId:string;clientName:string;google:boolean;github:boolean;nonce:string}){const links=[input.google?`<a data-provider="Google" href="/auth/google/start?login_state=${encodeURIComponent(input.stateId)}">Continue with Google</a>`:"",input.github?`<a data-provider="GitHub" href="/auth/github/start?login_state=${encodeURIComponent(input.stateId)}">Continue with GitHub</a>`:""].join("");return `<!doctype html><html><head><meta name="viewport" content="width=device-width"><meta name="color-scheme" content="light"><title>Sign in to x402 wallet</title><style>body{font:16px system-ui;max-width:480px;margin:10vh auto;padding:24px;background:#fff;color:#171717}a{display:block;box-sizing:border-box;width:100%;margin:12px 0;padding:14px;background:#fff;color:#171717!important;-webkit-text-fill-color:#171717;text-align:center;text-decoration:none;border:1px solid #bbb;border-radius:10px;font:inherit;touch-action:manipulation}a:active{background:#f1f1f1}a.busy{pointer-events:none;opacity:.65}.status{min-height:24px;color:#444}.status:empty{display:none}small{color:#666}</style></head><body><h1>Sign in to x402 wallet</h1><p>Continue to authenticate for <strong>${html(input.clientName)}</strong>.</p>${links}<p class="status" id="status" role="status" aria-live="polite"></p><small>You will review the MCP client and return address before access is granted.</small><script nonce="${input.nonce}">document.querySelectorAll("a[data-provider]").forEach(function(link){link.addEventListener("click",function(){link.classList.add("busy");document.getElementById("status").textContent="Opening "+link.dataset.provider+"…";});});</script></body></html>`;}
function consentPage(input:{consentToken:string;clientName:string;redirectOrigin:string;nonce:string}){return `<!doctype html><html><head><meta name="viewport" content="width=device-width"><meta name="color-scheme" content="light"><title>Authorize x402 wallet</title><style>body{font:16px system-ui;max-width:480px;margin:10vh auto;padding:24px;background:#fff;color:#171717}code{overflow-wrap:anywhere}button{display:block;box-sizing:border-box;width:100%;margin:12px 0;padding:14px;border:1px solid #171717;border-radius:10px;font:inherit;cursor:pointer;touch-action:manipulation}.allow{background:#171717;color:#fff!important;-webkit-text-fill-color:#fff}.deny{background:#fff;color:#171717!important;-webkit-text-fill-color:#171717}form.busy button{pointer-events:none;opacity:.65}.status{min-height:24px;color:#444}.status:empty{display:none}.status.busy:before{content:"";display:inline-block;width:14px;height:14px;margin-right:9px;vertical-align:-2px;border:2px solid #bbb;border-top-color:#171717;border-radius:50%;animation:spin .8s linear infinite}@keyframes spin{to{transform:rotate(360deg)}}small{color:#666}</style></head><body><h1>Authorize x402 wallet</h1><p><strong>${html(input.clientName)}</strong> is requesting access to your x402 wallet.</p><p>Access will return to <code>${html(input.redirectOrigin)}</code>.</p><p>Permission: preview and make Base USDC x402 payments within the server limits.</p><form id="consent-form" method="post" action="/oauth/consent"><input type="hidden" name="consent_token" value="${html(input.consentToken)}"><button class="allow" type="submit" name="decision" value="allow">Allow access</button><button class="deny" type="submit" name="decision" value="deny">Cancel</button></form><p class="status" id="status" role="status" aria-live="polite"></p><small>Continue only if you recognize this client and return address.</small><script nonce="${input.nonce}">var form=document.getElementById("consent-form"),status=document.getElementById("status");form.addEventListener("submit",function(event){if(form.dataset.submitted==="true"){event.preventDefault();return;}form.dataset.submitted="true";form.classList.add("busy");status.classList.add("busy");status.textContent=event.submitter&&event.submitter.value==="deny"?"Cancelling…":"Connecting to the MCP client…";setTimeout(function(){status.textContent="Still working — keep this window open…";},8000);});</script></body></html>`;}
