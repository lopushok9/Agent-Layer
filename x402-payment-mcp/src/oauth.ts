...
export function oauthRouter(config: Config, store: Store, tokens: TokenService) {
  ...
  router.get("/oauth/authorize", asyncRoute(async (req, res) => {
    ...
    const nonce=randomToken(16);secureHtml(res,nonce).send(loginPage({stateId,clientName:client.clientName,google:Boolean(config.GOOGLE_CLIENT_ID),github:Boolean(config.GITHUB_CLIENT_ID),nonce}));
  }));
...
async function showConsent(res:Response,store:Store,config:Config,state:Awaited<ReturnType<typeof consumeCallbackState>>,userId:string){
  ...
  const nonce=randomToken(16);secureHtml(res,nonce).send(consentPage({consentToken,clientName:client.clientName,redirectOrigin:new URL(state.redirectUri).origin,nonce}));
}
...
function secureHtml(res:Response,nonce:string){return res.set("Cache-Control","no-store").set("Content-Security-Policy",`default-src 'none'; style-src 'unsafe-inline'; script-src 'nonce-${nonce}'; form-action 'self'; frame-ancestors 'none'; base-uri 'none'`).set("X-Frame-Options","DENY").set("Referrer-Policy","no-referrer").type("html");}
