...
export function oauthRouter(config: Config, store: Store, tokens: TokenService) {
  ...
  router.get("/oauth/authorize", asyncRoute(async (req, res) => {
    ...
    const nonce=randomToken(16);secureHtml(res,nonce,"'none'").send(loginPage({stateId,clientName:client.clientName,google:Boolean(config.GOOGLE_CLIENT_ID),github:Boolean(config.GITHUB_CLIENT_ID),nonce}));
  }));
  ...
  router.get("/auth/github/start", asyncRoute(async(req,res)=>startProvider(req,res,store,tokens,config,"github")));
  ...
  if(googleConfig){
    router.get("/auth/google/callback", asyncRoute(async (req, res) => {
      const state = await consumeCallbackState(req, store,tokens,"google");
      if(typeof req.query.error==="string")return redirectOAuthError(res,state,providerError(req.query.error),"Google sign-in was not completed");
      ...
      const userId = await store.upsertIdentity("google", profile.sub, stringOrNull(profile.name), stringOrNull(profile.email));await showConsent(res, store, config, state, userId);
    }));
  }
  ...
  if(githubConfig){
    router.get("/auth/github/callback", asyncRoute(async (req, res) => {
      const state = await consumeCallbackState(req, store,tokens,"github");
      if(typeof req.query.error==="string")return redirectOAuthError(res,state,providerError(req.query.error),"GitHub sign-in was not completed");
      const code = requiredQuery(req, "code");
      ...
      const userId = await store.upsertIdentity("github", String(profile.id), stringOrNull(profile.name ?? profile.login), stringOrNull(profile.email));await showConsent(res, store, config, state, userId);
    }));
  }
...
async function startProvider(req:Request,res:Response,store:Store,tokens:TokenService,config:Config,provider:"google"|"github"){
  ...
  if(!await store.hasLoginState(stateId))return oauthJsonError(res,400,"invalid_request","authorization request is invalid or expired");
  const providerState=await tokens.providerState(stateId,provider);
  return res.set("Cache-Control","no-store").redirect(providerAuthorizationUrl(provider,providerState,config));
}
async function consumeCallbackState(req:Request,store:Store,tokens:TokenService,provider:"google"|"github"){
  const providerState=requiredQuery(req,"state");const stateId=await tokens.verifyProviderState(providerState,provider);const row=await store.consumeLoginState(stateId);if(!row)throw new Error("invalid or expired login state");return row;
}
async function showConsent(res:Response,store:Store,config:Config,state:Awaited<ReturnType<typeof consumeCallbackState>>,userId:string){
  const client=await store.getClient(state.clientId);if(!client)throw new Error("OAuth client no longer exists");
  const consentToken=await store.createPendingConsent(state,userId,config.AUTH_CODE_TTL_SECONDS);
  const redirectOrigin=new URL(state.redirectUri).origin;const nonce=randomToken(16);secureHtml(res,nonce,`'self' ${redirectOrigin}`).send(consentPage({consentToken,clientName:client.clientName,redirectOrigin,nonce}));
}
function finishLogin(res:Response,grant:import("./store.js").AuthorizationCode,code:string){const redirect=new URL(grant.redirectUri);redirect.searchParams.set("code",code);redirect.searchParams.set("state",grant.state);res.set("Cache-Control","no-store").redirect(303,redirect.toString());}
function redirectOAuthError(res:Response,state:{redirectUri:string;state:string},error:string,description:string){const redirect=new URL(state.redirectUri);redirect.searchParams.set("error",error);redirect.searchParams.set("error_description",description);redirect.searchParams.set("state",state.state);return res.set("Cache-Control","no-store").redirect(303,redirect.toString());}
...
function secureHtml(res:Response,nonce:string,formAction:string){return res.set("Cache-Control","no-store").set("Content-Security-Policy",`default-src 'none'; style-src 'unsafe-inline'; script-src 'nonce-${nonce}'; form-action ${formAction}; frame-ancestors 'none'; base-uri 'none'`).set("X-Frame-Options","DENY").set("Referrer-Policy","no-referrer").type("html");}
function providerError(value:string){return value==="access_denied"?"access_denied":"temporarily_unavailable";}
function html(value:string){return value.replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#39;");}
function loginPage(input:{stateId:string;clientName:string;google:boolean;github:boolean;nonce:string}){
  const links=[input.google?`<a data-provider="Google" href="/auth/google/start?login_state=${encodeURIComponent(input.stateId)}">Continue with Google</a>`:"",input.github?`<a data-provider="GitHub" href="/auth/github/start?login_state=${encodeURIComponent(input.stateId)}">Continue with GitHub</a>`:""].join("");
  return `<!doctype html><html><head><meta name="viewport" content="width=device-width"><meta name="color-scheme" content="light"><title>Sign in · AgentLayer</title><style>${pageStyles()}a.action{display:flex;color:#111!important;-webkit-text-fill-color:#111;background:#fff;border:1px solid #d6d6d1}a.action:hover{border-color:#9b9b95;background:#fafaf8}a.action:active{background:#f1f1ed}a.busy{pointer-events:none;opacity:.58}</style></head><body><main>${brand()}<div class="eyebrow">X402 wallet</div><h1>Sign in to continue</h1><p class="intro">Connect your identity to <strong>${html(input.clientName)}</strong>.</p><div class="actions">${links.replaceAll("<a data-provider=","<a class=\"action\" data-provider=")}</div><p class="status" id="status" role="status" aria-live="polite"></p><p class="footnote">You will review the client and return address before access is granted.</p><script nonce="${input.nonce}">document.querySelectorAll("a[data-provider]").forEach(function(link){link.addEventListener("click",function(){link.classList.add("busy");document.getElementById("status").textContent="Opening "+link.dataset.provider+"…";});});</script></main></body></html>`;
}
function consentPage(input:{consentToken:string;clientName:string;redirectOrigin:string;nonce:string}){
  return `<!doctype html><html><head><meta name="viewport" content="width=device-width"><meta name="color-scheme" content="light"><title>Authorize · AgentLayer</title><style>${pageStyles()}button{border:0}.allow{background:#111;color:#fff!important;-webkit-text-fill-color:#fff}.allow:hover{background:#292929}.deny{background:#fff;color:#111!important;-webkit-text-fill-color:#111;border:1px solid #d6d6d1}.deny:hover{border-color:#9b9b95;background:#fafaf8}form.busy button{pointer-events:none;opacity:.58}.status.busy:before{content:"";display:inline-block;width:13px;height:13px;margin-right:9px;vertical-align:-2px;border:2px solid #c6c6c0;border-top-color:#111;border-radius:50%;animation:spin .8s linear infinite}@keyframes spin{to{transform:rotate(360deg)}}</style></head><body><main>${brand()}<div class="eyebrow">Authorization request</div><h1>Allow wallet access?</h1><p class="intro"><strong>${html(input.clientName)}</strong> wants to use your AgentLayer x402 wallet.</p><dl><div><dt>Returns to</dt><dd><code>${html(input.redirectOrigin)}</code></dd></div><div><dt>Permission</dt><dd>Preview and make Base USDC x402 payments within your configured limits.</dd></div></dl><form class="actions" id="consent-form" method="post" action="/oauth/consent"><input type="hidden" name="consent_token" value="${html(input.consentToken)}"><button class="action allow" type="submit" name="decision" value="allow">Allow access</button><button class="action deny" type="submit" name="decision" value="deny">Cancel</button></form><p class="status" id="status" role="status" aria-live="polite"></p><p class="footnote">Continue only if you recognize this client and return address.</p><script nonce="${input.nonce}">var form=document.getElementById("consent-form"),status=document.getElementById("status");form.addEventListener("submit",function(event){if(form.dataset.submitted==="true"){event.preventDefault();return;}form.dataset.submitted="true";form.classList.add("busy");status.classList.add("busy");status.textContent=event.submitter&&event.submitter.value==="deny"?"Cancelling…":"Connecting to the MCP client…";setTimeout(function(){status.textContent="Still working — keep this window open…";},8000);});</script></main></body></html>`;
}
function brand(){return `<div class="brand"><svg aria-hidden="true" viewBox="0 0 48 48"><path d="M4 42 20.8 7.8A4.8 4.8 0 0 1 25.1 5h4.2l7.4 15H29l-5-9.9L8.3 42H4Zm10.2-4.8 3.1-6.2h22.4l3.1 6.2H14.2Zm5-10 3-6h10.4l3 6H19.2Zm-7 14h32.6l1 2a2 2 0 0 1-1.8 2.9h-7.2l-1.9-3.9H12.2Z"/></svg><span>AgentLayer</span></div>`;}
function pageStyles(){return `*{box-sizing:border-box}html{background:#f4f4f1}body{min-height:100vh;margin:0;padding:24px;display:grid;place-items:center;color:#141414;background:#f4f4f1;font-family:Inter,ui-sans-serif,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;font-size:15px;line-height:1.5;-webkit-font-smoothing:antialiased}main{width:min(100%,440px);padding:34px;background:#fff;border:1px solid #e2e2dd;border-radius:18px;box-shadow:0 18px 48px rgba(20,20,20,.07)}.brand{display:flex;align-items:center;gap:10px;margin-bottom:42px;font-size:17px;font-weight:700;letter-spacing:-.035em}.brand svg{width:29px;height:29px;fill:#151515}.eyebrow{margin:0 0 10px;color:#6b6b66;font-size:11px;font-weight:700;letter-spacing:.13em;text-transform:uppercase}h1{margin:0;color:#111;font-size:28px;line-height:1.13;font-weight:680;letter-spacing:-.035em}.intro{margin:14px 0 28px;color:#555550}.intro strong{color:#171717;font-weight:650}.actions{display:grid;gap:10px;margin:0}.action{min-height:50px;width:100%;align-items:center;justify-content:center;padding:13px 18px;border-radius:9px;font:inherit;font-weight:650;text-align:center;text-decoration:none;cursor:pointer;touch-action:manipulation;transition:background .15s,border-color .15s,opacity .15s}dl{margin:0 0 28px;border-top:1px solid #e7e7e2}dl div{padding:16px 0;border-bottom:1px solid #e7e7e2}dt{margin-bottom:5px;color:#74746e;font-size:11px;font-weight:700;letter-spacing:.08em;text-transform:uppercase}dd{margin:0;color:#292926;font-size:14px}code{font-family:"SFMono-Regular",Consolas,"Liberation Mono",monospace;font-size:12px;overflow-wrap:anywhere}.status{min-height:22px;margin:16px 0 0;color:#555550;font-size:13px}.status:empty{display:none}.footnote{margin:24px 0 0;color:#85857f;font-size:12px;line-height:1.55}@media(max-width:520px){body{padding:0;background:#fff;place-items:start center}main{padding:28px 22px;border:0;border-radius:0;box-shadow:none}.brand{margin-bottom:36px}}`;}
