...
- separate local EVM custody runtime
...
- `transfer_evm_native`
- `transfer_evm_token`
- `swap_evm_lifi_cross_chain_tokens` - EVM-origin LI.FI routes from Ethereum/Base/Arc to Ethereum/Base/Arc/Solana when LI.FI returns a route.
- `swap_solana_lifi_cross_chain_tokens` - Solana-origin LI.FI routes from Solana to Ethereum/Base/Arc when LI.FI returns a route.
...
- install Node dependencies for `wdk-evm-wallet`
...
For the local EVM backend (`backend=wdk_evm_local`), the lifecycle is:
...
- switching EVM networks does not mix balances across chains
