"""Smoke test for Houdini SPL deposit address resolution.

If Houdini returns a deposit owner address instead of a pre-created token account,
the backend should derive/create the ATA and send the transfer there.
"""
...
async def main() -> None:
    ...
    recipient_owner = SolanaLocalKeypairSigner(bytes(range(1, 33))).address
    ...
    recipient_owner_pubkey = Pubkey.from_string(recipient_owner)
    ...
    async def fake_account_exists(address: str, *, rpc_url):
        calls.append(("account_exists", address))
        if address == sender_ata:
            return True
        if address == recipient_owner:
            return False
        if address == recipient_ata:
            return False
    ...
    async def fake_send_transaction(*, transaction_base64: str, rpc_url):
        calls.append(("send_transaction", bool(transaction_base64)))
    ...
    try:
        ...
        result = await backend._send_houdini_exact_spl_deposit(
            recipient_token_account=recipient_owner,
            mint=USDC_MINT,
    ...
    assert result["requested_deposit_address"] == recipient_owner
    assert result["deposit_owner_address"] == recipient_owner
    assert result["deposit_address_interpretation"] == "owner_address"
    assert result["recipient_token_account"] == recipient_ata
    assert result["recipient_token_account_created"] is True
    assert result["confirmed"] is True
    assert ("account_exists", recipient_owner) in calls
    assert ("account_exists", recipient_ata) in calls
