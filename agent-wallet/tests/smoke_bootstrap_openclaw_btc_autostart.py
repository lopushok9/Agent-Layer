"""Smoke test for auto-starting the local WDK BTC service during bootstrap."""
...
RUNNER = ROOT / "tests" / "_fake_wdk_btc_service_runner.py"


def main() -> None:
    temp_home = Path("/tmp/openclaw-btc-bootstrap-autostart-smoke")
    if temp_home.exists():
        shutil.rmtree(temp_home)
    install_test_sealed_secrets(
        temp_home,
        boot_key="test-boot-key-for-btc-bootstrap-autostart-smoke",
        master_key="test-master-key-for-btc-bootstrap-autostart-smoke",
        approval_secret="test-approval-secret-for-btc-bootstrap-autostart-smoke",
    )

    fake_root = temp_home / "fake-wdk-btc-wallet"
    ...
    completed = subprocess.run(
        ...
        text=True,
        env=env,
        input="bootstrap-btc-autostart-password\n",
    )
    ...
    try:
        ...
        assert plugin_config["wdkBtcServiceUrl"] == service_url
        assert plugin_config["backend"] == "wdk_btc_local"
    finally:
        os.kill(pid, signal.SIGTERM)

    print("smoke_bootstrap_openclaw_btc_autostart: ok")
